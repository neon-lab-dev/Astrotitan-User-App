...
import PremiumRequiredModal from '../../../../components/AstrologersPage/PremiumRequiredModal/PremiumRequiredModal';
...
const AstrologerDetailsScreen = () => {
  ...
  const { data, isLoading, refetch } = useGetAstrologerByIdQuery(id, {
    skip: !id,
  });
            ...
            <View style={styles.overlayContent}>
              <SatoshiText style={styles.name}>
                {astrologer?.displayName ||
                  `${astrologer?.firstName || ''} ${
                    astrologer?.lastName || ''
                  }`}
              </SatoshiText>

              <SansText style={styles.sub}>
                {astrologer?.areaOfPractice?.join(', ') || 'Vedic Astrology'}
              </SansText>
            </View>
          </View>
            ...
            <View style={styles.statsRow}>
              ...
              <View style={styles.statBox}>
                ...
                <SansText style={styles.statLabel}>
                  Based on {astrologer?.reviews?.length || 0} reviews
                </SansText>
              </View>
              ...
              <View style={styles.statBox}>
                <SatoshiText style={styles.statValue}>
                  {astrologer?.experience + ' Years' || '0 Years'}
                </SatoshiText>
              ...
              <View>
                ...
                <View style={styles.tagRow}>
                  {astrologer?.consultLanguages?.length > 0
                    ? astrologer.consultLanguages.map((lang: string) => (
                        <View key={lang} style={styles.tag}>
                          <SansText style={styles.tagText}>{lang}</SansText>
                        </View>
                      ))
              ...
              {astrologer?.areaOfPractice?.length > 0 && (
                <View>
                  <ContentSection
                    ...
                    descriptionColor="#4A4A4A"
                  >
                    <SansText>Choose the area you need guidance in</SansText>
                  </ContentSection>

                  <View style={styles.tagRow}>
                    {astrologer?.areaOfPractice?.length > 0
                      ? astrologer.areaOfPractice.map((item: string) => (
                          <View key={item} style={styles.tag}>
                            <SansText style={styles.tagText}>{item}</SansText>
                          </View>
                        ))
                      : null}
                  </View>
